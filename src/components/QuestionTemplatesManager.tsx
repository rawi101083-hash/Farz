          ...
          {editingTemplate ? (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300">الأسئلة (بحد أقصى 4)</label>
                {newQuestions.map((q, idx) => (
                  <div key={idx} className="relative">
                    <span className="absolute right-4 top-4 text-slate-400 font-bold">{idx + 1}-</span>
                    <input
                      type="text"
                      value={q}
                      onChange={(e) => {
                      ...
                      className="w-full pl-4 pr-10 py-3.5 rounded-xl border-[2px] border-b-[4px] border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 focus:border-primary dark:focus:border-primary focus:border-b-primary dark:focus:border-b-primary outline-none text-sm transition-all font-medium"
