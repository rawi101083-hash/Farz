...
if (startIdx !== -1) {
  ...
  if (!newContent.includes('import { ChevronDown')) {
    newContent = newContent.replace('import { ', 'import { ChevronDown, ');
  }
  
  fs.writeFileSync('src/components/JobApplication.tsx', newContent);
  console.log('REPLACED', startIdx, endIdx);
} else {
