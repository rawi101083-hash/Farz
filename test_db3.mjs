...
async function run() {
  const { data: jobs, error: err1 } = await supabase.from('jobs').select('id, company_id').order('created_at', { ascending: false }).limit(5);
  console.log("Recent Jobs:", jobs, err1);

  const { data: apps, error: err2 } = await supabase.from('applicants').select('id, decision, ai_justification, job_id, created_at').order('created_at', { ascending: false }).limit(5);
