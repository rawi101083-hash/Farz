...
if (content.includes(targetContent)) {
  ...
  // Let's try matching line by line ignoring whitespace
  const fileLines = content.split('\\n');
  let startIdx = -1;
  for (let i = 0; i < fileLines.length; i++) {
    if (fileLines[i].includes('name="expectedSalary"')) {
      // found the input, backtrack
      startIdx = i - 7;
      break;
    }
  }
  ...
  if (startIdx >= 0) {
    fileLines.splice(startIdx, 16, ...replacementLines);
